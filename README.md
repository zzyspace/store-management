...
统一登录后台 `/store`，包含优惠券列表、连续扫码批量发放和批量核销。沿用报账后台浅深主题与导航样式。数据独立保存在 SQLite，不调用外部发券平台。
...
- 类型由券码自动识别；发放采用B版两步流程：先扫码并逐张确认，再填写赠送原因、操作人及发放时间并确认发放。第一步不校验隐藏填写项，也不提交发放请求。第二步上方为发放信息，下方完整展示本批券码，每批1至50张，可移除、不可编辑。返回上一步或继续扫码保留填写内容；清单为空时不能下一步或发放。发放时间按上海时区输入，接口必须含显式时区。
...
- 已发放或同批重复券逐张返回失败，其余有效券正常保存。部分成功后只保留失败券及原因供移除或重试；网络结果不明时锁定内容及步骤切换，并使用原批次重试。刷新页面或取消发放丢弃当前草稿。
- 新券为未核销；通过顶部“核销优惠券”进入批量核销，扫码只加入清单，填写操作人与核销时间并提交后才修改状态。核销每批1至50张，沿用券码格式、门店与只读清单规则。发放和核销草稿分别保存，关闭对应弹窗即清空。
- 核销操作人默认当前账号显示名，核销时间默认当前上海时间，均可修改且必填。未来时间或无效时间整批拒绝；未发放、已核销、跨店、格式无效、同批重复及早于该券发放时间逐券失败，其他有效券照常核销。成功券从待核销清单移除，失败券保留原因供修改或重试。
- 发放操作人和核销操作人分开保存，实际创建及核销账号独立留痕；核销后不支持修改、删除或撤销。列表采用B版紧凑布局，点击券条目查看完整原因、发放和核销操作人与时间；手机显示底部详情面板，桌面显示侧边详情面板，支持复制完整券码。已移除原每行单张核销按钮。
...
`coupon_issue_batches` 表以账号和 `requestId` 为唯一键，保存请求摘要及结果，与有效券记录在同一事务内提交。原批次重试返回原结果，同一标识对应不同内容返回409；每次重试仍校验当前权限。系统级数据库异常整批回滚。结果不明时使用原请求重试，收到确定结果后对失败券发起新的 `requestId`。旧单张发放接口保留原校验规则，历史券码不进行格式迁移。

核销批次使用独立的 `coupon_redeem_batches` 表，响应结构沿用逐券 `results` 并将成功计数命名为 `redeemedCount`。公共字段或权限错误不核销任何券；同批业务失败互不影响，系统级错误回滚当前整批。并发核销通过事务和条件更新保证同一券只成功一次。券记录新增 `redeemedOperator`；SQLite通过幂等迁移增加可空 `redeemed_operator` 列，历史缺失姓名返回null并显示“—”，不覆盖 `operator` 或推断旧核销人。旧单张核销接口继续支持 `{ store }`，采用当前账号显示名及服务器当前时间，并检查不得早于发放时间。
