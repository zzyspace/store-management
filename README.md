...
统一登录后台 `/store`，包含优惠券列表、连续扫码批量激活和批量核销。沿用报账后台浅深主题与导航样式。数据独立保存在 SQLite，不调用外部发券平台。
...
- `coupon:view`：查看当前门店全部优惠券；`coupon:issue`：激活；`coupon:redeem`：核销。后两者依赖查看权限。
...
- 批量激活的券码格式为 `门店代码-券类型-数字编号`：`FUZZY` / `FUZZYQZ` / `PEANUT` 对应上述三家门店，`ZY` 为赠饮券，`100` 为100元代金券。例如 `FUZZYQZ-ZY-2026101`。必须大写，编号不限定长度且保留前导零，完整券码最多200字符；跨店券码拒绝加入。
- 类型由券码自动识别；激活采用B版两步流程：先扫码并逐张确认，再填写赠送原因、操作人及激活时间并确认激活。第一步不校验隐藏填写项，也不提交激活请求。第二步上方为激活信息，下方完整展示本批券码，每批1至50张，可移除、不可编辑。返回上一步或继续扫码保留填写内容；清单为空时不能下一步或激活。激活时间按上海时区输入，接口必须含显式时区。
- 激活与核销共用扫码页，顶部展示用途、门店和已确认数量，区分启动、扫描中、等待确认和相机异常状态。优先启动后置相机，券码确认卡片固定在相机画面下方，仅暂停识别，实时视频持续播放；结束扫码或离开页面释放相机。页面转入后台时释放相机，返回后手动重试并保留已确认券码。浏览器需要 HTTPS（本机 localhost/回环地址亦可）及相机权限。jsQR 1.4.0 从本服务提供，画面在浏览器中解码、不上传。
- 已激活或同批重复券逐张返回失败，其余有效券正常保存。部分成功后只保留失败券及原因供移除或重试；网络结果不明时锁定内容及步骤切换，并使用原批次重试。刷新页面或取消激活丢弃当前草稿。
- 新券为未核销；通过顶部“核销优惠券”进入A版单页核销：空清单显示扫码入口，扫码后清单在上、操作人与核销时间在下，底部按钮显示本批数量。扫码只加入清单，填写并提交后才修改状态；清单为空时禁用提交。核销每批1至50张，沿用券码格式、门店与只读清单规则。激活和核销草稿分别保存，关闭对应弹窗即清空。
- 核销操作人默认当前账号显示名，核销时间默认当前上海时间，均可修改且必填。未来时间或无效时间整批拒绝；未激活、已核销、跨店、格式无效、同批重复及早于该券激活时间逐券失败，其他有效券照常核销。成功券从待核销清单移除，失败券保留原因供修改或重试。
- 激活操作人和核销操作人分开保存，实际创建及核销账号独立留痕；核销后不支持修改或撤销；有删除权限的账号可删除测试记录后重新激活券码。列表采用B版紧凑布局，点击券条目查看完整原因、激活和核销操作人与时间；手机显示底部详情面板，桌面显示侧边详情面板，支持复制完整券码。已移除原每行单张核销按钮。
...
| `POST /store/api/coupons/redeem-batch` | requestId（UUID v4）、store、codes（1至50个券码）、operator、redeemedAt；返回逐券 results、redeemedCount、failedCount，200 |
...
`coupon_issue_batches` 表以账号和 `requestId` 为唯一键，保存请求摘要及结果，与有效券记录在同一事务内提交。原批次重试返回原结果，同一标识对应不同内容返回409；每次重试仍校验当前权限。系统级数据库异常整批回滚。结果不明时使用原请求重试，收到确定结果后对失败券发起新的 `requestId`。旧单张激活接口保留原校验规则，历史券码不进行格式迁移。

核销批次使用独立的 `coupon_redeem_batches` 表，响应结构沿用逐券 `results` 并将成功计数命名为 `redeemedCount`。公共字段或权限错误不核销任何券；同批业务失败互不影响，系统级错误回滚当前整批。并发核销通过事务和条件更新保证同一券只成功一次。券记录新增 `redeemedOperator`；SQLite通过幂等迁移增加可空 `redeemed_operator` 列，历史缺失姓名返回null并显示“—”，不覆盖 `operator` 或推断旧核销人。旧单张核销接口继续支持 `{ store }`，采用当前账号显示名及服务器当前时间，并检查不得早于激活时间。
...
回滚先移除或恢复共享入口及导航，再恢复服务的 `current` 链接和网关代码；不要删除新券数据库。账号库扩大后的 CHECK 对旧代码兼容，可保留新增 store 授权数据；仅在确认迁移失败且无后续有效写入时使用备份恢复，避免覆盖新的账号变更。业务数据库在 release 目录之外，回退代码不丢失已激活和核销记录。

### 删除单张优惠券

账号管理 → 门店管理 → 优惠券管理可独立勾选“删除优惠券”（`coupon:delete`，依赖 `coupon:view`）。现有授权及角色模板不会自动增加此权限。详情仅向有权限的账号显示“删除当前券码”，确认后调用 `DELETE /store/api/coupons/:id`，JSON 请求体为 `{ "store": "fuzzy" }`。后端按当前账号权限和门店范围校验。

删除采用逻辑删除，未核销和已核销券均可删除；券码退出列表，恢复为未激活状态；可用新批次重新激活，再进行核销。数据库保留原记录、删除时间和真实删除账号，以及批次幂等结果；旧批次重试仅返回原批次结果，不会恢复券码或改变重新激活后的状态。
